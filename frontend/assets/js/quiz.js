...
function mostrarTelaConcluida(notas, mensagem) {
    ...
    resumo.innerHTML = `
        <div class="nota-card destaque-perfil">
            <strong>${notas.perfil_resultado || 'Perfil não definido'}</strong>
            <span>Perfil preditivo</span>
        </div>
        <div class="nota-card destaque-indice">
            <strong>${notas.indice_preditivo ?? 'N/A'}</strong>
            <span>Índice preditivo</span>
        </div>
        <div class="nota-card">
...
function gerarCertificadoPDF(notas) {
    ...
    doc.setFontSize(10);
    doc.setTextColor(0, 40, 85);
    doc.text(`Perfil Preditivo Dominante: ${notas.perfil_resultado || 'Não definido'}`, 46, resumoY + 10);
    doc.text(`Índice Preditivo Geral: ${notas.indice_preditivo ?? 'N/A'}`, w - 110, resumoY + 10);
    ...
    doc.save(`Certificado_Perfil_${usuario.nome.replace(/\s+/g, '_')}.pdf`);
}
